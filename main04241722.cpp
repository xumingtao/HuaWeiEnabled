...
vector<int>CalcuId(vector<int>path)
{
	...
	for (int j = 0; j < path.size() - 1; j++)
	{
		...
		//WayId.push_back(RandomId.size() == 0 ? min_id : RandomId[rand() % RandomId.size()]);
...
void PathCmp(vector<int>path)
{
	...
	else if (!StoreId.empty())						//表示此时求出的最短路径中无合适的通道以供使用
	{
		...
		for (int t = 0; t < StoreId.size(); t++)
		{
			cout << "Stored：" << StoreId[t] << endl;
			w[StoreId[t] * 2] = 0XFFFF;
			w[StoreId[t] * 2 + 1] = 0XFFFF;
		}
		...
		dijkstra(begin);
		vector<int>_pathway = get_path(end, begin);
		printTwoPointResult(_pathway);
		//begin = _pathway.front();
		//end=
